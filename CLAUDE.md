...
find ~/.claude/plugins/cache/forge-local -maxdepth 4 -name "plugin.json" -path "*/forge/*" 2>/dev/null \
  | sort -V | tail -1 | sed 's|/.claude-plugin/plugin.json||' \
  | grep . \
...
Efforts are stored under `.forge/efforts/` in two subdirectories:
- `active/`  — efforts currently in progress
- `archive/` — closed efforts kept for historical reference

Active efforts are listed in `.forge/efforts/README.md` (start here to find an effort by ID
or title). Effort artifacts are in `.forge/efforts/active/<id>/`:
...
Archived efforts are moved to `.forge/efforts/archive/<id>/` when closed. If a name
collision exists, the directory is suffixed with the closure date:
`archive/<id>-<YYYY-MM-DD>/`.
<!-- forge:managed:end -->
